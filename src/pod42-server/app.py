...
async def lambda_handler(event):
    ...
    chain = MyVectorDBQAWithSourcesChain(
        combine_document_chain=doc_chain, vectorstore=store, k=5
    )
    ...
    resp = await asyncio.to_thread(
        requests.post, HGE_URL, data=json.dumps(jsonBody), headers=HEADERS
    )

    response = {"statusCode": response_code, "body": resp.json()}
